...
* \`reason\`: 주관적 칭찬이나 단순 행동 요약을 금지합니다. 반드시 제공된 데이터 내에서 [증거: PR #번호 및 링크 / 커밋해시]를 정확히 명시하고, 소프트웨어 엔지니어링 전문 용어(예: 시간/공간 복잡도, 모듈 결합도 및 응집도, 상태 관리, 메모리 누수, 렌더링 최적화, 동시성 제어 등 해당 도메인의 핵심 키워드)를 사용하여 시스템에 입힌 아키텍처적 및 엔지니어링적 임팩트를 논증하십시오.
...
* \`example\`: \`improvement\`에서 지적한 기술적 한계와 트레이드오프를 기반으로, 피평가자가 향후 코드 리뷰나 사내 기술 공유 채널(Slack, Issue 등)에서 실제 팀원들과 논의할 때 즉시 복사·붙여넣기해서 쓸 수 있는 실전 소통 템플릿(GitHub Comment / Slack Message 형태)을 구체적인 기술 키워드를 채워 직접 작성하십시오. (예: "팀원 여러분, 현재 구조는 X의 한계가 있어 다음 단계로 Y 패턴 도입을 제안합니다. 코드 레벨에서는..." 과 같이 동료 엔지니어의 액션을 이끌어내는 실전 소통 양식으로 작성되어야 합니다.)
